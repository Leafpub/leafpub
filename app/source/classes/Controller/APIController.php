...
use Leafpub\Cache;
use Leafpub\Error;
use Leafpub\Events\Application\MailCompose;
use Leafpub\Events\Application\MailSend;
use Leafpub\Importer;
use Leafpub\Language;
use Leafpub\Leafpub;
use Leafpub\Mailer;
use Leafpub\Mailer\Mail\AddressFactory;
use Leafpub\Mailer\Mail\MailFactory;
...
class APIController extends Controller
{
    ...
    public function recover($request, $response, $args)
    {
        ...
        try {
            $emailData = MailFactory::create([
                ...
                'message' => Language::term('a_password_reset_has_been_requested_for_this_account') . "\n\n" .
                    $user['name'] . ' — ' . $user['slug'] . "\n\n" .
    ...
    public function deleteUser($request, $response, $args)
    {
        ...
        try {
            ...
            if ($args['slug'] === Session::user()['slug']) {
                Session::logout();
            }
        } catch (\Exception $e) {
    ...
    public function updateSettings($request, $response, $args)
    {
        ...
        $settings = [
            ...
            'show_dashboard' => $params['showDashboard'] === 'on' ? 'on' : 'off',
    ...
    public function getLocater($request, $response, $args)
    {
        ...
        if ($isDashboard) {
            foreach (Widget::getWidgets() as $widget) {
                if (stristr($widget['name'], $params['query'])) {
                    $items[] = [
        ...
        foreach (Admin::getMenuItems() as $nav) {
            if (stristr($nav['title'], $params['query'])) {
                $items[] = [
                    'title' => $nav['title'],
                    'description' => null,
                    'link' => $nav['link'],
